...
- **Correction datée (2026-06-12, après relecture critique)** : la phrase « le cantus de Fux est rejeté par la grammaire de la cloche / le matériau mélodique est timbre-relatif » de l'entrée précédente était SURINTERPRÉTÉE. Le rejet est indépendant de τ_F (vérifié 0,05→1,20) : il vient de R3-argmax (frontière basse, 307 c) × convention Φ(unisson)=∞ × espace d'action v1 (voix au-dessus, compas ≤ octave), pas du paysage de fusion. Énoncé retenu : **dans v1, la grammaire de la cloche n'admet pas de cadence descendante** (conclusion naturelle ascendante vers la tierce-de-cloche). Matrice de contrôle 2×2 ajoutée (l'harmonique accepte cantus ET miroir ; la cloche v1 n'accepte que le miroir). Détail et variantes à tester : `ETAPE3-GRAPHE.md` §3.
...
- **2026-06-12 — Étape 4 ENGAGÉE (option écoute validée par l'utilisateur après discussion du statut épistémique — « ça sonne » n'est pas le but, c'est le tribunal).** Stimuli générés (`etape4_stimuli.py`, graine fixe : 6 fichiers mono ~13 s, noms neutres A-F pour écoute en aveugle partiel) : les deux chemins légaux dérivés + croisement décisif quintes-parallèles / tierces-de-cloche-parallèles à timbre échangé. **Prédictions E1-E4 enregistrées AVANT toute écoute** dans `ETAPE4-ECOUTE.md` (protocole : juger la SÉPARABILITÉ, pas l'agrément — l'hygiène C≠Φ vaut à l'écoute ; confusions possibles déclarées). Synthèse restée en Python (suffisante pour la falsification) ; Overtone/Clojure ne se rejouera qu'à l'étape 5 (instrument).
- **2026-06-12 (17 h 31) — Étape 4 CONCLUE : 4/4 prédictions d'écoute dans le sens prédit, aucune réfutation.** Trois itérations de protocole, chacune consignée (`ETAPE4-ECOUTE.md` §7) : v1 tenues statiques = échec de plancher CONSIGNÉ COMME DONNÉE (sans articulation temporelle, pas de voix du tout — le temporel passe d'« extension » à « précondition ») ; v2 notation absolue = tâche inadaptée (micro-datum : l'auditeur a entendu l'alternance unisson/intervalle du chemin légal sans la connaître) ; v3 choix forcé par paires = verdict. **Résultats (§5)** : E1 quintes-parallèles-vs-légal sur harmonique : nette (le connu reproduit à l'oreille) ; E2 légal-vs-tierces-parallèles sur cloche : confirmée ; **E3 mêmes quintes parallèles, timbre échangé : confirmée — l'interdit de quintes est timbre-dépendant À L'OREILLE** ; E4 croisement symétrique : confirmée, faible. Limites déclarées : n=1, auditeur informé (aveugle sur l'ordre des moitiés seulement), falsification informelle. **Statut : P5 perceptif corroboré — la boucle complète du projet est fermée une fois : spectre mesuré publié → Φ figée → grammaire dérivée → oreille.** « Ça doit sonner » honoré au niveau falsification.
- **2026-09-07 — DIAGRAMME DE PHASE (extension de l'étape 3, prévue au CADRAGE §7) : les 4 prédictions D1-D4 confirmées.** Voir `DIAGRAMME-PHASE.md` (+ `famille_raide.py`, `diagramme_phase.py`). Axe : la corde raide `f_n = n·f0·√(1+B·n²)`, un paramètre physique continu et **mesurable sur de vrais instruments**, ancré aux deux bouts (à `B=0`, exactement le timbre de contrôle qui reproduit Fux ; à `B` grand, un régime de type barre). **Prédictions commitées AVANT tout calcul** (commit `b3be72f`, sans code ni résultat — l'historique git fait foi). Résultats : **D1** ancrage exact ; **D2 l'octave part vers l'aigu quand B croît** (+0,86 c au médium de piano, +3,4 c aux basses de droit, +27 c au-delà) — signe, ordre de grandeur et dépendance corrects vs le phénomène de Railsback documenté depuis 1938, **alors qu'aucune donnée d'accordage n'est jamais entrée dans le projet** (contrôle de cohérence externe que Φ pouvait rater ; ce n'est PAS une explication nouvelle du phénomène, qui se dérive déjà par la dissonance sensorielle) ; **D3** 8 bifurcations après durcissement du critère (le premier test comptait 57 changements, dont 49 n'étaient que de la dérive continue — correction à charge documentée) ; **D4** dans la limite raide, les pics tombent sur les **carrés de rationnels** (25/16 à 1,6 c près, 16/9 à 4,1 c) et **l'interdit de parallèles d'octave disparaît**.
- **Le fait le plus intéressant du balayage** : la 1re bifurcation est à `B ≈ 7,7·10⁻³`, **un ordre de grandeur au-dessus des cordes de piano les plus raides**. Toute la plage des instruments à cordes réels est donc dans **une seule phase, celle de Fux** : la structure de la grammaire (3 interdits — unisson, quinte, octave) y est invariante, seules les positions dérivent de quelques cents. Hypothèse que ça suggère, à manier avec prudence : si les règles ont paru universelles, c'est peut-être que tous les instruments qui les ont produites vivaient du même côté de la première bifurcation — et qu'il faut la raideur d'une barre ou d'une cloche pour en sortir.
- **Modification déclarée de la chaîne** : compas d'analyse extensible au-delà de 1200 c (paramètre `cmax`, défaut inchangé) — sans quoi une octave étirée sortirait de la fenêtre. Étapes 3 et 4 vérifiées inchangées au bit près.
- **État du projet à cette date : les étapes 0-4 du plan de passation sont faites, plus le diagramme de phase du CADRAGE §7.** Reste, par ordre de valeur : (a) P6 propre (récepteur rhinolophe réel — le résultat épistémologique substrat/saillance n'a jamais été refait depuis le noyau jetable) ; (b) consolidation pour thèse/article (le matériel est dans les six documents d'étape + dépôt rejouable) ; (c) étape 5, l'instrument (différée par ce plan même comme « esthétiquement central, scientifiquement redondant ») ; (d) générer plus d'un auditeur / contrebalancer si on veut durcir le perceptif ; (e) la dynamique temporelle (préparation/résolution — désormais motivée par DEUX faits : l'inharmonicité de la petite boîte vit dans l'attaque, et v1-statique ne produit pas l'explanandum sans articulation).
