...
- **2026-06-12 — Étape 4 ENGAGÉE (option écoute validée par l'utilisateur après discussion du statut épistémique — « ça sonne » n'est pas le but, c'est le tribunal).** Stimuli générés (`etape4_stimuli.py`, graine fixe : 6 fichiers mono ~13 s, noms neutres A-F pour écoute en aveugle partiel) : les deux chemins légaux dérivés + croisement décisif quintes-parallèles / tierces-de-cloche-parallèles à timbre échangé. **Prédictions E1-E4 enregistrées AVANT toute écoute** dans `ETAPE4-ECOUTE.md` (protocole : juger la SÉPARABILITÉ, pas l'agrément — l'hygiène C≠Φ vaut à l'écoute ; confusions possibles déclarées). **En attente : les notes d'écoute de l'utilisateur** (§5 du protocole). Synthèse restée en Python (suffisante pour la falsification) ; Overtone/Clojure ne se rejouera qu'à l'étape 5 (instrument).
